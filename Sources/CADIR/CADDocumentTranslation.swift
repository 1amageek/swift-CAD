...
private extension Curve3D {
    func translatingSources(
        ...
        case let .analytic(curve):
            return .analytic(try curve.translatingSources(
                by: vector,
                tolerance: tolerance
            ))
        ...
        case let .implicit(curve):
            var firstSurface = curve.firstSurface
            firstSurface.controlPoints = firstSurface.controlPoints.map { row in
                row.map { $0 + vector }
            }
            var secondSurface = curve.secondSurface
            secondSurface.controlPoints = secondSurface.controlPoints.map { row in
                row.map { $0 + vector }
            }
            return .implicit(try CertifiedImplicitIntersectionCurve(
        ...
        case let .surfaceLift(curve):
            return .surfaceLift(SurfaceLiftCurve3D(
                surface: curve.surface.translatingSources(by: vector),
                parameterCurve: curve.parameterCurve
            ))
        }
    }
}
...
private extension AnalyticCurve3D {
    func translatingSources(
        ...
        case let .hyperbola(curve):
            return .hyperbola(Hyperbola3D(
                center: curve.center + vector,
                normal: curve.normal,
                transverseAxis: curve.transverseAxis,
                transverseRadius: curve.transverseRadius,
                conjugateRadius: curve.conjugateRadius
            ))
        case let .parabola(curve):
            return .parabola(Parabola3D(
                vertex: curve.vertex + vector,
                normal: curve.normal,
                axis: curve.axis,
                focalLength: curve.focalLength
            ))
        case let .planeTorus(curve):
            return .planeTorus(try CertifiedPlaneTorusIntersectionCurve(
                planeSurface: curve.planeSurface.translatingSources(by: vector),
                torusSurface: curve.torusSurface.translatingSources(by: vector),
                componentKind: curve.componentKind,
                lowerMinorAngle: curve.lowerMinorAngle,
                upperMinorAngle: curve.upperMinorAngle,
                tolerance: tolerance
            ))
        }
    }
...
private extension Surface3D {
    func translatingSources(by vector: Vector3D) -> Surface3D {
        switch self {
        case .plane(var plane):
            plane.origin = plane.origin + vector
            return .plane(plane)
        case .cylinder(var cylinder):
            cylinder.origin = cylinder.origin + vector
            return .cylinder(cylinder)
        case let .analytic(surface):
            return .analytic(surface.translatingSources(by: vector))
        case .bSpline(var surface):
            surface.controlPoints = surface.controlPoints.map { row in
                row.map { $0 + vector }
            }
...
private extension AnalyticSurface3D {
    func translatingSources(by vector: Vector3D) -> AnalyticSurface3D {
        switch self {
        case let .plane(origin, normal):
            return .plane(origin: origin + vector, normal: normal)
        case let .cylinder(origin, axis, radius):
            return .cylinder(origin: origin + vector, axis: axis, radius: radius)
        case let .cone(apex, axis, halfAngle):
            return .cone(apex: apex + vector, axis: axis, halfAngle: halfAngle)
