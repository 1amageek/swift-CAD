    ...
    struct Builder {
        ...
        mutating func buildLoop(_ pointer: Int, role: LoopRole, surface: Surface3D) throws -> LoopID {
            ...
            for _ in 0..<edgeCount {
                ...
                cursor += 5
                guard pcurveCount == 1, cursor + 1 < values.count else {
                    throw unsupported("IGES exact coedges require exactly one parameter-space curve.")
                }
                _ = try booleanAt(values, cursor, label: "loop isoparametric flag")
                let pcurvePointer = try integerAt(values, cursor + 1, label: "loop p-curve pointer")
                cursor += 2
                ...
                let modelParameterCurve = try parameterCurve(
                    pcurvePointer,
        ...
        mutating func buildCurve(_ pointer: Int) throws -> (id: CurveID, trim: CurveTrim?) {
            ...
            case 104:
                let values = entity.parameters
                guard entity.form == 1, values.count == 12 else {
                    throw unsupported("IGES conic #\(pointer) is not a supported ellipse.")
                }
                ...
                guard let transformation else {
                    throw unsupported("IGES ellipse #\(pointer) requires a defining transformation.")
                }
                ...
                let localStart = Point3D(
                    x: lengthUnit.toInternal(try numberAt(values, 8, label: "ellipse start X")),
                    y: lengthUnit.toInternal(try numberAt(values, 9, label: "ellipse start Y")),
                    z: lengthUnit.toInternal(try numberAt(values, 7, label: "ellipse Z"))
                )
                let localEnd = Point3D(
                    x: lengthUnit.toInternal(try numberAt(values, 10, label: "ellipse end X")),
                    y: lengthUnit.toInternal(try numberAt(values, 11, label: "ellipse end Y")),
                    z: localStart.z
                )
