...
| Operation parity | UI tools and AI agents must call the same typed operation requests. | FeatureOperation is the shared IR, generated curves flow through the same evaluation context as sketch curves, the SwiftCAD builder exposes bridge curve, curve edit, sweep, PolySpline, FaceLoopOffset, and FaceKnife operations, and the SwiftCAD facade exposes intent-filtered snap queries and document-level sketch dimension solving. | Add agent command schemas and keep public SwiftCAD-level builder APIs aligned with every new feature operation. |
