...
| Editable dimensions | Dimension labels must bind to exact references and feed the same solver/operation path used by agents. | Use selection measurements as the input boundary for general distance and angle dimensions, then connect them to editable constraints. |
...
| Agent parity | An agent must not use a separate geometric approximation from the UI. | `CADAgentMeasurementQuery` calls the same selection measurement evaluator used by public SwiftCAD APIs. |
