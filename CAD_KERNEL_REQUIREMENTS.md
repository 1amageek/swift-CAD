...
| Snapping and dimensions | Accurate sketching and construction workflows need exact snapping and editable dimensions. | Sketch dimensions exist, solver-ready constraint graph equations preserve dimension target values, sketch dimension evaluation reports measured values and residuals, and kernel query APIs expose evaluated curve endpoints, midpoints, parameters, tangents, curvature, B-spline spans, knots, control points, B-rep edge endpoints/frames/projection, surface UVN/knot/span/CV queries, closest-point surface projection, direction-based surface projection, planar trimmed-face projection bounds, and intent-filtered snap candidates for vertices, edges, and faces. | Kernel queries for endpoints, midpoints, centers, tangencies, projected points, construction planes, distances, angles, and editable dimension constraints. | Add non-planar trim-aware projection and editable dimension constraint solving. |
...
| Operation parity | UI tools and AI agents must call the same typed operation requests. | FeatureOperation is the shared IR, generated curves flow through the same evaluation context as sketch curves, the SwiftCAD builder exposes bridge curve and sweep operations, and the SwiftCAD facade exposes intent-filtered snap queries. | Add public SwiftCAD-level builder APIs and agent command schemas for every feature operation. |
