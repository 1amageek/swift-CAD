...
| Operation parity | UI tools and AI agents must call the same typed operation requests. | FeatureOperation is the shared IR, generated curves flow through the same evaluation context as sketch curves, the SwiftCAD builder exposes bridge curve and sweep operations, and the SwiftCAD facade exposes intent-filtered snap queries and document-level sketch dimension solving. | Add public SwiftCAD-level builder APIs and agent command schemas for every feature operation. |
