...
|---|---|---|---|---|
| Parametric curve model | Sketching, sweep paths, bridge curves, tangent snaps, and dimensions need editable curves with stable parameters. | Lines and circles are modeled. B-spline/NURBS curves are missing. | Lines, arcs, circles, Bezier, B-spline, and rational NURBS curves with evaluation, derivatives, domains, knots, spans, and weights. | Add rational curve primitives after rational surface evaluation is in place. |
...
| Continuity contracts | Bridge, fillet, blend, and surface matching need explicit G0/G1/G2/G3 targets rather than visual-only alignment. | Rupa has high-level continuity analysis, but the kernel lacks a shared continuity contract. | Typed curve and surface continuity evaluators with deviation metrics, tangent/curvature constraints, and solve-ready diagnostics. | Define kernel-level continuity requests once NURBS curve and surface derivatives are shared. |
