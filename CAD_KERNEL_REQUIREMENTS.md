...
| Topology and stable naming | Object, face, edge, vertex, CV, knot, span, and trim selection must survive feature recomputation and agent operations. | B-rep tables, generated persistent names, evaluated curve output tables, and curve subobject references exist for basic feature and curve selection. | Stable references for bodies, faces, edges, vertices, trims, surface spans, curve spans, knots, and control vertices. | Extend generated names and selection references to trims and surface subobjects. |
...
| Snapping and dimensions | Accurate sketching and construction workflows need exact snapping and editable dimensions. | Sketch dimensions exist, and kernel curve query APIs now expose evaluated curve endpoints, midpoints, parameters, tangents, curvature, B-spline spans, knots, and control points. | Kernel queries for endpoints, midpoints, centers, tangencies, projected points, construction planes, distances, angles, and editable dimension constraints. | Add surface/edge projection queries and editable dimension constraint solving. |
...
|---|---|---|---|
| Subobject selection | Bodies, faces, edges, vertices, curve spans, surface spans, knots, and CVs need stable references. | Bodies, faces, edges, vertices, generated persistent names, curve output references, curve span/knot/CV references, and evaluated curve tables exist for core feature outputs. | Extend references to trims, surface spans, surface knots, surface CVs, and generated surface outputs. |
...
| Surface editing | Sweep, loft, bridge, patch, PolySpline, continuity matching, UVN frames, and curvature diagnostics must be exact geometry first. | Rational B-spline surfaces, surface continuity checks, surface sampling, sweep, and PolySpline surface generation are partially available. | Add trimmed surfaces, curve-on-surface links, loft/bridge surface solvers, and diagnostic query APIs. |
