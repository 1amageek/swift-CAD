...
| Snapping and dimensions | Accurate sketching and construction workflows need exact snapping and editable dimensions. | Sketch dimensions exist, and kernel query APIs expose evaluated curve endpoints, midpoints, parameters, tangents, curvature, B-spline spans, knots, control points, B-rep edge endpoints/frames/projection, surface UVN/knot/span/CV queries, closest-point surface projection, and direction-based surface projection. | Kernel queries for endpoints, midpoints, centers, tangencies, projected points, construction planes, distances, angles, and editable dimension constraints. | Add trim-aware surface projection and editable dimension constraint solving. |
...
|---|---|---|---|
| Subobject selection | Bodies, faces, edges, vertices, curve spans, surface spans, knots, and CVs need stable references. | Bodies, faces, edges, vertices, generated persistent names, typed edge parameter references, curve output references, curve span/knot/CV references, face surface span/knot/CV references, evaluated curve tables, edge projection queries, and face-to-UV projection queries exist for core feature outputs. | Extend references to trims, generated surface outputs, and topology-to-parameter projection references. |
| Curve authoring | Lines, arcs, circles, Bezier, B-spline, rational NURBS, offsets, bridge curves, tangent snaps, and dimensions must share exact curve types. | Lines, circles, rational B-spline curves, sketch splines, curve continuity, bridge curve output, and exact/fallback curve query APIs are available. | Add editable knot/CV mutation operations, exact offsets, projection, trim, and sketch constraints over generated curves. |
| Surface editing | Sweep, loft, bridge, patch, PolySpline, continuity matching, UVN frames, and curvature diagnostics must be exact geometry first. | Rational B-spline surfaces, surface continuity checks, surface sampling, sweep, PolySpline surface generation, and surface query frames are partially available. | Add trimmed surfaces, curve-on-surface links, loft/bridge surface solvers, and diagnostic projection APIs. |
| Operation parity | UI tools and AI agents must call the same typed operation requests. | FeatureOperation is the shared IR, generated curves flow through the same evaluation context as sketch curves, and the SwiftCAD builder exposes bridge curve and sweep operations. | Add public SwiftCAD-level builder APIs and agent command schemas for every feature operation. |
