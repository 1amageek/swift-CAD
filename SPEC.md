...
samples before boundary-progress matching. For the current two-section guide
subset, one or more open guide curves with intermediate samples also insert
rail-following intermediate section rings so the generated side faces follow the
guides as a piecewise ruled B-spline surface. Sheet output
...
deformation beyond the current two-section guide subset, continuity controls,
and boolean target operations remain outside the current supported subset and
must fail explicitly when represented.
