...
@Suite("Exact translational Sweep command parity")
struct ExactTranslationalSweepCommandParityTests {
    @Test(.timeLimit(.minutes(1)))
    func builderCommandAndPersistenceProduceIdenticalExactSweep() throws {
        ...
        let path = try builder.sketch(on: .yz, named: "Exact arc path") { sketch in
            sketch.arc(
        ...
        let sweepFeatureID = try builder.sweep(
            ...
            named: "Exact translational Sweep"
        )
        let builderDocument = try builder.build(
            name: "Exact translational Sweep parity"
        )
        ...
        guard case .face = try #require(
            ...
            Issue.record("Expected exact Sweep side face topology.")
            return
        }
