...
@Suite("Exact point-guide Sweep command parity")
struct ExactPointGuideSweepCommandParityTests {
    @Test(.timeLimit(.minutes(1)))
    func builderCommandAndPersistenceProduceIdenticalExactPointGuideSweep() throws {
        ...
        let profile = try builder.sketch(
            on: .xy,
            named: "Point-guide Sweep profile"
        ) { sketch in
            sketch.rectangle(
        ...
        let path = try builder.sketch(
            on: .yz,
            named: "Straight point-guide path"
        ) { sketch in
            _ = sketch.line(
                from: point(0.0, 0.0),
                to: point(0.0, 0.05)
        ...
        let guide = try builder.sketch(
            on: .yz,
            named: "Exact point guide"
        ) { sketch in
            _ = sketch.line(
                from: point(0.01, 0.0),
                to: point(0.02, 0.05)
        ...
        let sweepFeatureID = try builder.sweep(
            ...
            guides: [guide.featureID],
            options: SweepOptions(
                alignment: .parallel,
                guideMethod: .point
            ),
            named: "Exact point-guide Sweep"
        )
        let builderDocument = try builder.build(
            name: "Exact point-guide Sweep parity"
        )
        ...
        guard case .face = try #require(
            ...
            Issue.record("Expected exact point-guide Sweep side topology.")
            return
        }
        ...
        #expect(abs((endVertices.map(\.x).min() ?? 0.0) + 0.04) <= tolerance.distance)
        #expect(abs((endVertices.map(\.x).max() ?? 0.0) - 0.04) <= tolerance.distance)
        #expect(abs((endVertices.map(\.y).min() ?? 0.0) + 0.02) <= tolerance.distance)
        #expect(abs((endVertices.map(\.y).max() ?? 0.0) - 0.02) <= tolerance.distance)
